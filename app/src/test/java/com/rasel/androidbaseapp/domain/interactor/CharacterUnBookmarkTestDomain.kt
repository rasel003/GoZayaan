...
class CharacterUnBookmarkTestDomain : DomainBaseTest() {
    ...
    @Test
    fun `set unbookmark character with id should return success with response code`() = runTest {
            // Arrange (Given)
            val characterId = 1L
    ...
    @Test
    fun `set unbookmark character with id should return fail with response code`() = runTest {
            // Arrange (Given)
            val characterId = 1L
            whenever(characterRepository.setCharacterUnBookMarked(characterId)) doReturn flow {
                emit(
